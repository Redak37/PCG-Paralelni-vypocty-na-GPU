...
constexpr int COMPUTE_VELOCITIES = 1;
constexpr int UPDATE_PARTICLES = 2;
constexpr int COMPUTE_COM = 3;
constexpr int UPDATE_COM = 4;
...
struct Particles
{
  ...
  void copyToCPUAsync()
  {
    #pragma acc update host(pos_x[n], pos_y[n], pos_z[n], vel_x[n], vel_y[n], vel_z[n]) wait(COMPUTE_VELOCITIES) async(UPDATE_PARTICLES)
  }
};// end of Particles
//----------------------------------------------------------------------------------------------------------------------
