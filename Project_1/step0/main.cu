...
int main(int argc, char **argv)
{
  ...
  */
  ...
  cudaMalloc<float>(&tmp_vel.x, N * sizeof(float));
  cudaMalloc<float>(&tmp_vel.y, N * sizeof(float));
  cudaMalloc<float>(&tmp_vel.z, N * sizeof(float));
  
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  //                                       FILL IN: memory transfers (step 0)                                         //
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

  cudaMemcpy(particles_gpu.pos_x, particles_cpu.pos_x, N * sizeof(float),
             cudaMemcpyHostToDevice);
  cudaMemcpy(particles_gpu.pos_y, particles_cpu.pos_y, N * sizeof(float),
             cudaMemcpyHostToDevice);
  cudaMemcpy(particles_gpu.pos_z, particles_cpu.pos_z, N * sizeof(float),
             cudaMemcpyHostToDevice);
  cudaMemcpy(particles_gpu.vel_x, particles_cpu.vel_x, N * sizeof(float),
             cudaMemcpyHostToDevice);
  cudaMemcpy(particles_gpu.vel_y, particles_cpu.vel_y, N * sizeof(float),
             cudaMemcpyHostToDevice);
  cudaMemcpy(particles_gpu.vel_z, particles_cpu.vel_z, N * sizeof(float),
             cudaMemcpyHostToDevice);
  cudaMemcpy(particles_gpu.weight, particles_cpu.weight, N * sizeof(float),
             cudaMemcpyHostToDevice);
  ...
  for(int s = 0; s < steps; s++)
  {
    ...
    calculate_collision_velocity<<<gridDim, blockDim>>>(particles_gpu, tmp_vel, N, dt);
    calculate_gravitation_velocity<<<gridDim, blockDim>>>(particles_gpu, tmp_vel, N, dt);
    update_particle<<<gridDim, blockDim>>>(particles_gpu, tmp_vel, N, dt);
  ...
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  float4 comOnGPU;
  cudaMemcpy(particles_cpu.pos_x, particles_gpu.pos_x, N * sizeof(float),
             cudaMemcpyDeviceToHost);
  cudaMemcpy(particles_cpu.pos_y, particles_gpu.pos_y, N * sizeof(float),
             cudaMemcpyDeviceToHost);
  cudaMemcpy(particles_cpu.pos_z, particles_gpu.pos_z, N * sizeof(float),
             cudaMemcpyDeviceToHost);
  cudaMemcpy(particles_cpu.vel_x, particles_gpu.vel_x, N * sizeof(float),
             cudaMemcpyDeviceToHost);
  cudaMemcpy(particles_cpu.vel_y, particles_gpu.vel_y, N * sizeof(float),
             cudaMemcpyDeviceToHost);
  cudaMemcpy(particles_cpu.vel_z, particles_gpu.vel_z, N * sizeof(float),
             cudaMemcpyDeviceToHost);
